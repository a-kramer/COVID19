...
int covid19_vf(realtype t, N_Vector y_, N_Vector f_, void *params)
    ...
    NV_Ith_S(f_,1) = -quick_recovery+initial_infection+outbreak-critical-infectious_death;
...
int covid19_jac(long N_, realtype t,
    ...
    DENSE_ELEM(jac_, 0, 0) = -a-Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0));
    DENSE_ELEM(jac_, 0, 1) = -( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))-Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0));
    DENSE_ELEM(jac_, 0, 2) = -Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0));
    DENSE_ELEM(jac_, 0, 3) = -Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0));
    DENSE_ELEM(jac_, 0, 4) = -Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0));
    DENSE_ELEM(jac_, 1, 0) = a;
    DENSE_ELEM(jac_, 1, 1) = -l-c-qq;
...
int covid19_jacp(int N_, realtype t,
    ...
    DENSE_ELEM(jacp_, 0, 0) = l*Infectious*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*( distancing_on-1.0);
    DENSE_ELEM(jacp_, 0, 1) = -Exposed;
    DENSE_ELEM(jacp_, 0, 3) = -( distancing_on*r0_dist-r0*( distancing_on-1.0))*Infectious*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit);
    DENSE_ELEM(jacp_, 0, 8) = -l*distancing_on*Infectious*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit);
    DENSE_ELEM(jacp_, 0, 10) = ( l*r0-l*r0_dist)*Infectious*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit);
    DENSE_ELEM(jacp_, 0, 11) = Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0));
    ...
    DENSE_ELEM(jacp_, 1, 7) = (1.0/8.0)*pow(Pi,-(1.0/2.0))*exp(-3.1250000000000000e-02*pow( g+t,2.0))*pow(2.0,(1.0/2.0))/Population*( -6.2500000000000000e-02*g+-6.2500000000000000e-02*t);
...
int covid19_sens(int Ns_, realtype t, N_Vector y_, N_Vector ydot_,
    ...
    NV_Ith_S(ySdot_,0) = -S0*( a+Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0)))-S3*Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))-Infectious*S4*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))-Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))*S2-S1*( ( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))+Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0)));
    NV_Ith_S(ySdot_,1) =  S0*a-( l+c+qq)*S1;
    NV_Ith_S(ySdot_,2) = -( m+q)*S2+S1*c;
    NV_Ith_S(ySdot_,3) =  m*S2+l*S1;
    NV_Ith_S(ySdot_,4) =  q*S2+S1*qq;

	switch (iS_) {
        case 0:
	            NV_Ith_S(ySdot_,0) += l*Infectious*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*( distancing_on-1.0);
	        break;
        case 1:
        ...
        case 2:
	        ...
	        break;
        case 3:
	            NV_Ith_S(ySdot_,0) += -( distancing_on*r0_dist-r0*( distancing_on-1.0))*Infectious*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit);
	            NV_Ith_S(ySdot_,1) += -Infectious;
        ...
        case 6:
	        ...
	        break;
        case 7:
	            NV_Ith_S(ySdot_,1) += (1.0/8.0)*pow(Pi,-(1.0/2.0))*exp(-3.1250000000000000e-02*pow( g+t,2.0))*pow(2.0,(1.0/2.0))/Population*( -6.2500000000000000e-02*g+-6.2500000000000000e-02*t);
	        break;
        case 8:
	            NV_Ith_S(ySdot_,0) += -l*distancing_on*Infectious*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit);
	        break;
        case 9:
	        break;
        case 10:
	            NV_Ith_S(ySdot_,0) += ( l*r0-l*r0_dist)*Infectious*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit);
	        break;
        case 11:
	            NV_Ith_S(ySdot_,0) += Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0));
	        break;
	 }
    return 0;
    }
...
int covid19_func_sens(realtype t, N_Vector y_, N_Vector * yS_, double* ret_, void *params)
    ...
    /* r0:  */
        ret_[0] = -yS__0_4-yS__0_1-yS__0_2-yS__0_3-yS__0_0;
        ret_[1] = yS__0_1;
        ret_[2] = yS__0_3;
        ret_[3] = yS__0_4;
        ret_[4] = yS__0_2;
    ...
    /* a:  */
        ...
        ret_[15] = yS__1_1*h;
    /* c:  */
        ret_[16] = -yS__2_4-yS__2_1-yS__2_2-yS__2_3-yS__2_0;
        ret_[17] = yS__2_1;
        ret_[18] = yS__2_3;
        ret_[19] = yS__2_4;
        ret_[20] = yS__2_2;
    ...
    /* m:  */
        ...
        ret_[39] = yS__4_1*h;
    /* q:  */
        ret_[40] = -yS__5_2-yS__5_3-yS__5_0-yS__5_4-yS__5_1;
        ret_[41] = yS__5_1;
        ret_[42] = yS__5_3;
        ret_[43] = yS__5_4;
        ret_[44] = yS__5_2;
    ...
    /* qq:  */
        ...
        ret_[55] = yS__6_1*h;
    /* g:  */
        ret_[56] = -yS__7_1-yS__7_2-yS__7_3-yS__7_0-yS__7_4;
        ret_[57] = yS__7_1;
        ret_[58] = yS__7_3;
        ret_[59] = yS__7_4;
        ret_[60] = yS__7_2;
        ...
        ret_[63] = yS__7_1*h;
    /* r0_dist:  */
        ret_[64] = -yS__8_3-yS__8_0-yS__8_4-yS__8_1-yS__8_2;
        ret_[65] = yS__8_1;
        ret_[66] = yS__8_3;
        ret_[67] = yS__8_4;
        ret_[68] = yS__8_2;
    ...
    /* h:  */
        ...
        ret_[77] = 0.0;
        ret_[78] = 0.0;
        ret_[79] =  yS__9_1*h+Infectious;
    /* distancing_on:  */
        ret_[80] = -yS__10_3-yS__10_0-yS__10_4-yS__10_1-yS__10_2;
        ret_[81] = yS__10_1;
        ret_[82] = yS__10_3;
        ret_[83] = yS__10_4;
        ret_[84] = yS__10_2;
    ...
    /* Susceptible_ConservedConst:  */
        ret_[88] = -yS__11_4-yS__11_1-yS__11_2-yS__11_3-yS__11_0+1.0;
        ret_[89] = yS__11_1;
        ret_[90] = yS__11_3;
        ret_[91] = yS__11_4;
        ret_[92] = yS__11_2;

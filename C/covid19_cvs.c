...
int covid19_vf(realtype t, N_Vector y_, N_Vector f_, void *params)
    ...
    b = l*r0;
    b_dist = l*r0_dist;
    B = -( distancing_on-1.0)*b+distancing_on*b_dist;
    ...
    NV_Ith_S(f_,1) = -infectious_death-quick_recovery+initial_infection+outbreak-critical;
    NV_Ith_S(f_,2) = -late_recovery-critical_death+critical;
    NV_Ith_S(f_,3) =  quick_recovery+late_recovery;
    NV_Ith_S(f_,4) =  infectious_death+critical_death;
    return 0;
    }
...
int covid19_jac(long N_, realtype t,
    ...
    DENSE_ELEM(jac_, 0, 0) =  ( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious-a;
    DENSE_ELEM(jac_, 0, 1) =  ( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious+( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist);
    DENSE_ELEM(jac_, 0, 2) = ( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious;
    DENSE_ELEM(jac_, 0, 3) = ( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious;
    DENSE_ELEM(jac_, 0, 4) = ( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious;
    DENSE_ELEM(jac_, 1, 0) = a;
    DENSE_ELEM(jac_, 1, 1) = -qq-l-c;
...
int covid19_jacp(int N_, realtype t,
    ...
    DENSE_ELEM(jacp_, 0, 0) = l*( distancing_on-1.0)*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*Infectious;
    DENSE_ELEM(jacp_, 0, 1) = -Exposed;
    DENSE_ELEM(jacp_, 0, 3) = ( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*Infectious*( ( distancing_on-1.0)*r0-distancing_on*r0_dist);
    DENSE_ELEM(jacp_, 0, 8) = -l*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*distancing_on*Infectious;
    DENSE_ELEM(jacp_, 0, 10) = -( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*( l*r0_dist-l*r0)*Infectious;
    DENSE_ELEM(jacp_, 0, 11) = -( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious;
    ...
    DENSE_ELEM(jacp_, 1, 7) = (1.0/8.0)*pow(Pi,-(1.0/2.0))*( -6.2500000000000000e-02*t+-6.2500000000000000e-02*g)*pow(2.0,(1.0/2.0))/Population*exp(-3.1250000000000000e-02*pow( t+g,2.0));
...
int covid19_sens(int Ns_, realtype t, N_Vector y_, N_Vector ydot_,
    ...
    NV_Ith_S(ySdot_,0) =  ( ( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious-a)*S0+( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious*S4+( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*S3*Infectious+S2*( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious+( ( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious+( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist))*S1;
    NV_Ith_S(ySdot_,1) =  S0*a-( qq+l+c)*S1;
    NV_Ith_S(ySdot_,2) = -S2*( m+q)+S1*c;
    NV_Ith_S(ySdot_,3) =  l*S1+S2*m;
    NV_Ith_S(ySdot_,4) =  S2*q+qq*S1;

	switch (iS_) {
        case 0:
	            NV_Ith_S(ySdot_,0) += l*( distancing_on-1.0)*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*Infectious;
	        break;
        case 1:
        ...
        case 2:
	        ...
	        break;
        case 3:
	            NV_Ith_S(ySdot_,0) += ( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*Infectious*( ( distancing_on-1.0)*r0-distancing_on*r0_dist);
	            NV_Ith_S(ySdot_,1) += -Infectious;
        ...
        case 6:
	        ...
	        break;
        case 7:
	            NV_Ith_S(ySdot_,1) += (1.0/8.0)*pow(Pi,-(1.0/2.0))*( -6.2500000000000000e-02*t+-6.2500000000000000e-02*g)*pow(2.0,(1.0/2.0))/Population*exp(-3.1250000000000000e-02*pow( t+g,2.0));
	        break;
        case 8:
	            NV_Ith_S(ySdot_,0) += -l*( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*distancing_on*Infectious;
	        break;
        case 9:
	        break;
        case 10:
	            NV_Ith_S(ySdot_,0) += -( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*( l*r0_dist-l*r0)*Infectious;
	        break;
        case 11:
	            NV_Ith_S(ySdot_,0) += -( l*( distancing_on-1.0)*r0-l*distancing_on*r0_dist)*Infectious;
	        break;
	 }
    return 0;
    }
...
int covid19_func(realtype t, N_Vector y_, realtype *func_, void *params)
    ...
    func_[6] = 1.0/( exp( -5.0000000000000000e+00*r0+5.0000000000000000e+00*r0_dist)+1.0);
    /* hospital_beds:  */
...
int covid19_func_sens(realtype t, N_Vector y_, N_Vector * yS_, double* ret_, void *params)
    ...
    /* r0:  */
        ret_[0] = -yS__0_0-yS__0_4-yS__0_1-yS__0_2-yS__0_3;
        ret_[1] = yS__0_1;
        ret_[2] = yS__0_3;
        ret_[3] = yS__0_4;
        ret_[4] = yS__0_2;
        ret_[5] = 0.0;
        ret_[6] = 5.0000000000000000e+00*1.0/pow( exp( -5.0000000000000000e+00*r0+5.0000000000000000e+00*r0_dist)+1.0,2.0)*exp( -5.0000000000000000e+00*r0+5.0000000000000000e+00*r0_dist);
        ret_[7] = h*yS__0_1;
    /* a:  */
        ret_[8] = -yS__1_2-yS__1_3-yS__1_0-yS__1_4-yS__1_1;
        ret_[9] = yS__1_1;
        ret_[10] = yS__1_3;
        ret_[11] = yS__1_4;
        ret_[12] = yS__1_2;
        ...
        ret_[15] = h*yS__1_1;
    /* c:  */
        ret_[16] = -yS__2_3-yS__2_0-yS__2_4-yS__2_1-yS__2_2;
        ret_[17] = yS__2_1;
        ret_[18] = yS__2_3;
        ret_[19] = yS__2_4;
        ret_[20] = yS__2_2;
        ...
        ret_[23] = h*yS__2_1;
    /* l:  */
        ret_[24] = -yS__3_2-yS__3_3-yS__3_0-yS__3_4-yS__3_1;
        ret_[25] = yS__3_1;
        ret_[26] = yS__3_3;
        ret_[27] = yS__3_4;
        ret_[28] = yS__3_2;
        ...
        ret_[31] = h*yS__3_1;
    /* m:  */
        ret_[32] = -yS__4_3-yS__4_0-yS__4_4-yS__4_1-yS__4_2;
        ret_[33] = yS__4_1;
        ret_[34] = yS__4_3;
        ret_[35] = yS__4_4;
        ret_[36] = yS__4_2;
        ...
        ret_[39] = h*yS__4_1;
    /* q:  */
        ret_[40] = -yS__5_4-yS__5_1-yS__5_2-yS__5_3-yS__5_0;
        ret_[41] = yS__5_1;
        ret_[42] = yS__5_3;
        ret_[43] = yS__5_4;
        ret_[44] = yS__5_2;
        ...
        ret_[47] = h*yS__5_1;
    /* qq:  */
        ret_[48] = -yS__6_3-yS__6_0-yS__6_4-yS__6_1-yS__6_2;
        ret_[49] = yS__6_1;
        ret_[50] = yS__6_3;
        ret_[51] = yS__6_4;
        ret_[52] = yS__6_2;
        ...
        ret_[55] = h*yS__6_1;
    /* g:  */
        ret_[56] = -yS__7_4-yS__7_1-yS__7_2-yS__7_3-yS__7_0;
        ret_[57] = yS__7_1;
        ret_[58] = yS__7_3;
        ret_[59] = yS__7_4;
        ret_[60] = yS__7_2;
        ...
        ret_[63] = h*yS__7_1;
    /* r0_dist:  */
        ret_[64] = -yS__8_2-yS__8_3-yS__8_0-yS__8_4-yS__8_1;
        ret_[65] = yS__8_1;
        ret_[66] = yS__8_3;
        ret_[67] = yS__8_4;
        ret_[68] = yS__8_2;
        ret_[69] = 0.0;
        ret_[70] = -5.0000000000000000e+00*1.0/pow( exp( -5.0000000000000000e+00*r0+5.0000000000000000e+00*r0_dist)+1.0,2.0)*exp( -5.0000000000000000e+00*r0+5.0000000000000000e+00*r0_dist);
        ret_[71] = h*yS__8_1;
    /* h:  */
        ret_[72] = -yS__9_4-yS__9_1-yS__9_2-yS__9_3-yS__9_0;
        ret_[73] = yS__9_1;
        ret_[74] = yS__9_3;
        ret_[75] = yS__9_4;
        ret_[76] = yS__9_2;
    ...
    /* distancing_on:  */
        ret_[80] = -yS__10_2-yS__10_3-yS__10_0-yS__10_4-yS__10_1;
        ret_[81] = yS__10_1;
        ret_[82] = yS__10_3;
        ret_[83] = yS__10_4;
        ret_[84] = yS__10_2;
    ...
    /* Susceptible_ConservedConst:  */
        ret_[88] = -yS__11_0-yS__11_4-yS__11_1-yS__11_2-yS__11_3+1.0;
        ret_[89] = yS__11_1;
        ret_[90] = yS__11_3;
        ret_[91] = yS__11_4;
        ret_[92] = yS__11_2;

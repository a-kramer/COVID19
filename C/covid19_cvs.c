 ...
 *  Generated on  6-Apr-2020 at 00:20
 */
...
int covid19_jac(long N_, realtype t,
    ...
    DENSE_ELEM(jac_, 0, 0) = -a-( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious;
    DENSE_ELEM(jac_, 0, 1) = -( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)-( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious;
    DENSE_ELEM(jac_, 0, 2) = -( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious;
    DENSE_ELEM(jac_, 0, 3) = -( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious;
    DENSE_ELEM(jac_, 0, 4) = -( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious;
...
int covid19_jacp(int N_, realtype t,
    ...
    DENSE_ELEM(jacp_, 0, 0) = ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*Infectious*( distancing_on-1.0)*l;
    DENSE_ELEM(jacp_, 0, 1) = -Exposed;
    DENSE_ELEM(jacp_, 0, 3) = ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*( r0*( distancing_on-1.0)-r0_dist*distancing_on)*Infectious;
    DENSE_ELEM(jacp_, 0, 7) = -( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*distancing_on*Infectious*l;
    DENSE_ELEM(jacp_, 0, 8) = ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*Infectious*( r0*l-r0_dist*l);
    DENSE_ELEM(jacp_, 0, 9) = ( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious;
    DENSE_ELEM(jacp_, 1, 1) = Exposed;
    DENSE_ELEM(jacp_, 1, 2) = -Infectious;
    DENSE_ELEM(jacp_, 1, 3) = -Infectious;
    DENSE_ELEM(jacp_, 1, 6) = (1.0/8.0)*exp(-3.1250000000000000e-02*pow( t+g,2.0))*pow(2.0,(1.0/2.0))*pow(Pi,-(1.0/2.0))/Population*( -6.2500000000000000e-02*t+-6.2500000000000000e-02*g);
...
int covid19_sens(int Ns_, realtype t, N_Vector y_, N_Vector ydot_,
    ...
    NV_Ith_S(ySdot_,0) = -S3*( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious-( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious*S2-S1*( ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)+( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious)-( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*S4*Infectious-( a+( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious)*S0;
    NV_Ith_S(ySdot_,1) = -( c+l)*S1+a*S0;
    NV_Ith_S(ySdot_,2) = -( m+q)*S2+c*S1;
    NV_Ith_S(ySdot_,3) =  m*S2+S1*l;
    NV_Ith_S(ySdot_,4) = q*S2;

	switch (iS_) {
        case 0:
	            NV_Ith_S(ySdot_,0) += ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*Infectious*( distancing_on-1.0)*l;
	        break;
        case 1:
        ...
        case 2:
	        ...
	        break;
        case 3:
	            NV_Ith_S(ySdot_,0) += ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*( r0*( distancing_on-1.0)-r0_dist*distancing_on)*Infectious;
	            NV_Ith_S(ySdot_,1) += -Infectious;
        ...
        case 5:
	        ...
	        break;
        case 6:
	            NV_Ith_S(ySdot_,1) += (1.0/8.0)*exp(-3.1250000000000000e-02*pow( t+g,2.0))*pow(2.0,(1.0/2.0))*pow(Pi,-(1.0/2.0))/Population*( -6.2500000000000000e-02*t+-6.2500000000000000e-02*g);
	        break;
        case 7:
	            NV_Ith_S(ySdot_,0) += -( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*distancing_on*Infectious*l;
	        break;
        case 8:
	            NV_Ith_S(ySdot_,0) += ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*Infectious*( r0*l-r0_dist*l);
	        break;
        case 9:
	            NV_Ith_S(ySdot_,0) += ( r0_dist*distancing_on*l-r0*( distancing_on-1.0)*l)*Infectious;
	        break;
	 }
    return 0;
    }
...
int covid19_func_sens(realtype t, N_Vector y_, N_Vector * yS_, double* ret_, void *params)
    ...
    /* a:  */
        ...
        ret_[9] = yS_12;
    /* c:  */
        ret_[10] = -yS_20-yS_24-yS_21-yS_22-yS_23;
        ret_[11] = yS_21;
        ret_[12] = yS_23;
        ret_[13] = yS_24;
        ret_[14] = yS_22;
    /* l:  */
        ...
        ret_[19] = yS_32;
    /* m:  */
        ret_[20] = -yS_43-yS_40-yS_44-yS_41-yS_42;
        ret_[21] = yS_41;
        ret_[22] = yS_43;
        ret_[23] = yS_44;
        ret_[24] = yS_42;
    ...
    /* g:  */
        ...
        ret_[34] = yS_62;
    /* r0_dist:  */
        ret_[35] = -yS_71-yS_72-yS_73-yS_70-yS_74;
        ret_[36] = yS_71;
        ret_[37] = yS_73;
        ret_[38] = yS_74;
        ret_[39] = yS_72;
    /* distancing_on:  */
        ret_[40] = -yS_83-yS_80-yS_84-yS_81-yS_82;
        ret_[41] = yS_81;
        ret_[42] = yS_83;
        ret_[43] = yS_84;
        ret_[44] = yS_82;
    /* Susceptible_ConservedConst:  */
        ret_[45] = -yS_94-yS_91-yS_92-yS_93-yS_90+1.0;
        ret_[46] = yS_91;
        ret_[47] = yS_93;
        ret_[48] = yS_94;
        ret_[49] = yS_92;

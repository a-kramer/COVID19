...
int covid19_jac(long N_, realtype t,
    ...
    DENSE_ELEM(jac_, 0, 0) = -a-Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0);
    DENSE_ELEM(jac_, 0, 1) = -( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0)-Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0);
    DENSE_ELEM(jac_, 0, 2) = -Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0);
    DENSE_ELEM(jac_, 0, 3) = -Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0);
    DENSE_ELEM(jac_, 0, 4) = -Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0);
...
int covid19_jacp(int N_, realtype t,
    ...
    DENSE_ELEM(jacp_, 0, 0) = ( distancing_on-1.0)*l*( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*Infectious;
    DENSE_ELEM(jacp_, 0, 1) = -Exposed;
    DENSE_ELEM(jacp_, 0, 3) = ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*( ( distancing_on-1.0)*r0-distancing_on*r0_dist)*Infectious;
    DENSE_ELEM(jacp_, 0, 8) = -l*( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*distancing_on*Infectious;
    DENSE_ELEM(jacp_, 0, 10) = -( l*r0_dist-l*r0)*( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*Infectious;
    DENSE_ELEM(jacp_, 0, 11) = Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0);
    ...
    DENSE_ELEM(jacp_, 1, 7) = (1.0/8.0)*pow(Pi,-(1.0/2.0))*pow(2.0,(1.0/2.0))*( -6.2500000000000000e-02*t+-6.2500000000000000e-02*g)*exp(-3.1250000000000000e-02*pow( t+g,2.0))/Population;
...
int covid19_sens(int Ns_, realtype t, N_Vector y_, N_Vector ydot_,
    ...
    NV_Ith_S(ySdot_,0) = -Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0)*S4-( a+Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0))*S0-S1*( ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0)+Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0))-S3*Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0)-S2*Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0);
    NV_Ith_S(ySdot_,1) =  S0*a-S1*( c+qq+l);
    NV_Ith_S(ySdot_,2) = -( q+m)*S2+S1*c;
    NV_Ith_S(ySdot_,3) =  S2*m+S1*l;
    NV_Ith_S(ySdot_,4) =  S1*qq+q*S2;

	switch (iS_) {
        case 0:
	            NV_Ith_S(ySdot_,0) += ( distancing_on-1.0)*l*( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*Infectious;
	        break;
        case 1:
        ...
        case 2:
	        ...
	        break;
        case 3:
	            NV_Ith_S(ySdot_,0) += ( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*( ( distancing_on-1.0)*r0-distancing_on*r0_dist)*Infectious;
	            NV_Ith_S(ySdot_,1) += -Infectious;
        ...
        case 6:
	        ...
	        break;
        case 7:
	            NV_Ith_S(ySdot_,1) += (1.0/8.0)*pow(Pi,-(1.0/2.0))*pow(2.0,(1.0/2.0))*( -6.2500000000000000e-02*t+-6.2500000000000000e-02*g)*exp(-3.1250000000000000e-02*pow( t+g,2.0))/Population;
	        break;
        case 8:
	            NV_Ith_S(ySdot_,0) += -l*( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*distancing_on*Infectious;
	        break;
        case 9:
	        break;
        case 10:
	            NV_Ith_S(ySdot_,0) += -( l*r0_dist-l*r0)*( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)*Infectious;
	        break;
        case 11:
	            NV_Ith_S(ySdot_,0) += Infectious*( l*distancing_on*r0_dist-( distancing_on-1.0)*l*r0);
	        break;
	 }
    return 0;
    }
...
int covid19_func_sens(realtype t, N_Vector y_, N_Vector * yS_, double* ret_, void *params)
    ...
    /* r0:  */
        ...
        ret_[4] = yS__0_2;
        ret_[5] = 0.0;
        ret_[6] = 5.0000000000000000e+00*exp( -5.0000000000000000e+00*r0+5.0000000000000000e+00*r0_dist)/pow( exp( -5.0000000000000000e+00*r0+5.0000000000000000e+00*r0_dist)+1.0,2.0);
        ret_[7] = h*yS__0_1;
    /* a:  */
    ...
    /* c:  */
        ...
        ret_[23] = h*yS__2_1;
    /* l:  */
        ret_[24] = -yS__3_4-yS__3_1-yS__3_2-yS__3_3-yS__3_0;
        ret_[25] = yS__3_1;
        ret_[26] = yS__3_3;
        ret_[27] = yS__3_4;
        ret_[28] = yS__3_2;
    ...
    /* r0_dist:  */
        ...
        ret_[68] = yS__8_2;
        ret_[69] = 0.0;
        ret_[70] = -5.0000000000000000e+00*exp( -5.0000000000000000e+00*r0+5.0000000000000000e+00*r0_dist)/pow( exp( -5.0000000000000000e+00*r0+5.0000000000000000e+00*r0_dist)+1.0,2.0);
        ret_[71] = h*yS__8_1;
    /* h:  */
    ...
    /* distancing_on:  */
        ret_[80] = -yS__10_1-yS__10_2-yS__10_3-yS__10_0-yS__10_4;
        ret_[81] = yS__10_1;
        ret_[82] = yS__10_3;
        ret_[83] = yS__10_4;
        ret_[84] = yS__10_2;

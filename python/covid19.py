...
def vectorfield(y_, t_, p):
    ...
    NORM = pow(Pi,(1.0/2.0))*sigma*pow(2.0,(1.0/2.0))
    Gaussian = EDIFFSQ/NORM
    b = l*r0
    b_dist = l*r0_dist
    B = -b*( distancing_on-1.0)+distancing_on*b_dist
    ...
    f_[1] = -critical-infectious_death-quick_recovery+initial_infection+outbreak
    f_[2] =  critical-late_recovery-critical_death
    f_[3] =  quick_recovery+late_recovery
    f_[4] =  infectious_death+critical_death
...
def jacobian(y_, t_, p_):
    ...
    jac_[0,0] = -( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))*Infectious-a
    jac_[0,1] = -( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))*( Krit+Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst)-( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))*Infectious

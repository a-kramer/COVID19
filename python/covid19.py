...
def vectorfield(y_, t_, p):
    ...
    f_[1] = -infectious_death-quick_recovery+initial_infection+outbreak-critical
...
def jacobian(y_, t_, p_):
    ...
    jac_[0,0] = -a-Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))
    jac_[0,1] = -( Recovered+Exposed+Dead+Infectious-Susceptible_ConservedConst+Krit)*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))-Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))
    jac_[0,2] = -Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))
    jac_[0,3] = -Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))
    jac_[0,4] = -Infectious*( l*distancing_on*r0_dist-l*r0*( distancing_on-1.0))
    jac_[1,0] = a
    jac_[1,1] = -qq-l-c
    jac_[2,1] = c
    jac_[2,2] = -m-q
